...
class FsmakerShell(cmd.Cmd):
    ...
    def binary_print(self, data, prefix=""):
        binary = ""
        line = ""
        for i in range(int(math.floor(len(data) / 2))):
    ...
    def dirents_print(self, dirents, prefix=""):
        ...
        if (len(direntlist) == 0):
            ...
            for dirent in direntlist:
                ...
                print("{4}{0:<{1}} {2:>{3}} {5}".format(dirent.name.decode(), maxlen, dirent.inode, len(str(maxinode)), prefix, msg))
    ...
    def do_inode(self, args):
        ...
        if (len(args) == 0):
            ...
            for arg in args:
                try:
                    inode = self._simdisk.get_inode(int(arg))
                except ValueError as e:
                    inode = self.open(arg)
                except api.S5fsException as e:
                    inode = self.open(arg)
                    ...
                    try:
                        ...
                        if (options.indirect and inode.get_type() in set([ api.S5_TYPE_DATA, api.S5_TYPE_DIR ]) and inode.get_indirect_blockno() != 0):
                            try:
                                ...
                                for i in range(api.S5_BLOCK_SIZE / 4):
    ...
    def complete_inode(self, text, line, begidx, endidx):
        ...
        try:
            string.atoi(text)
            res += [str(x) for x in range(self._simdisk.get_num_inodes()) if str(x).startswith(text)]
        except ValueError:
            None
