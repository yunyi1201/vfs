 ...
 * entry or on return. `res_vnode` must not be locked on return. Return via
 * `name` and `namelen` the basename of path.
 *
 ...
 *
 * Hints:
 *  - When *calling* namev_dir(), if it is unclear what to pass as the `base`,
 * you should use `curproc->p_cwd` (think about why this makes sense).
 ...
 * An additional note: namelen is used in conjunction with the string you set
 * name to. For example, you'll use functions like namev_lookup(), which through
 * the pipeline, uses strncmp to compare the first n letters in the name string
 * (like "ccc/") to another string. It's helpful to think about how namev_dir()
 * gets used in other functions. Also, look at ramfs_lookup() which gets called
 * by namev_lookup() when you're doing VFS. namev_lookup() --> ramfs_lookup()
 * --> name_match() to see how name and namelen get used! Therefore, name should
 * be set to the last part of the path string. 1.) a/b/c ==> name = c, namelen =
 * 1 2.) a/b/cc/ ==> name = cc/, namelen = 2 3.) a/b/c//// ==> name = c////,
 * namelen = 1
 */
long namev_dir(vnode_t *base, const char *path, vnode_t **res_vnode,
  ...
  do {
    ...
    token = namev_tokenize(&search, &token_len);
    while (search && *search == '/') {
      search++;
    }

    if (!token || !token_len || !search || !*search) {
      break;
    }
    vlock(cur);
    long ret = namev_lookup(cur, token, token_len, &next);
    vunlock(cur);
    if (ret < 0) {
      vput(&cur);
      return ret;
...
long namev_open(vnode_t *base, const char *path, int oflags, int mode,
  ...
  vnode_t *dir;
  long ret;
  ret = namev_dir(base, path, &dir, &name, &namelen);
  if (ret < 0) {
    return ret;
  }
  ...
  // name ending with '/' implies that the basename is a directory
  if (name[namelen] == '/' && (oflags & O_CREAT)) {
    vput(&dir);
    return -EINVAL;
  }
  ...
  vlock(dir);
  ret = namev_lookup(dir, name, namelen, res_vnode);

  if (ret == 0) {
    vunlock(dir);
    vput(&dir);
    if (S_ISREG((*res_vnode)->vn_mode) && name[namelen] == '/') {
      vput(res_vnode);
      return -ENOTDIR;
    }
    return 0;
  ...
  if ((oflags & O_CREAT) && ret == -ENOENT) {
    if (dir->vn_ops->mknod) {
      ret = dir->vn_ops->mknod(dir, name, namelen, mode, devid, res_vnode);
    } else {
      ret = -ENOTSUP;
    }
  }
